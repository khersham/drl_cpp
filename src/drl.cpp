...
torch::Tensor sample(const torch::Tensor& mean, const torch::Tensor& scale) {
...
void VPGBuffer::store(const torch::Tensor& obs, const torch::Tensor& act, const torch::Tensor& rew,
                      const torch::Tensor& val, const torch::Tensor& logp) {
    if (ptr < max_size) {
        obs_buf[ptr] = obs;
        act_buf[ptr] = act;
        rew_buf[ptr] = rew;
...
torch::Tensor compute_loss_v(const torch::Tensor& obs, const torch::Tensor& ret, ActorCritic& ac){
...
torch::Tensor compute_loss_pi(const torch::Tensor& obs, const torch::Tensor& act, 
                              const torch::Tensor& adv, ActorCritic& ac){
...
void update(VPGBuffer& buf, ActorCritic& ac, torch::optim::Adam& pi_optimizer, 
            torch::optim::Adam& v_optimizer, const int& critic_iter, const torch::Device& device) {
